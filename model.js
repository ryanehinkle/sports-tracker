...
const DEFAULTS={l5:60,l10:55,h2h:0,current:50,previous:0,targetShare:0,carryShare:0,opportunityShare:0,dvpMin:0,dvpSample:2,legOddsMin:-500,legOddsMax:500,parlayOddsMin:100,parlayOddsMax:350,legsMin:2,legsMax:4,weights:{recent:30,season:22,h2h:12,usage:14,matchup:14,value:8}};
...
function buildUsage(){
  ...
  for(const p of state.players){
    ...
    state.usage.set(String(p.id),{
      ...
      targets:own.targets,carries:own.carries
    });
  }
}
...
function controls(){
  ...
  return{hit:hit,targetShare:Number($("targetShare").value)||0,carryShare:Number($("carryShare").value)||0,opportunityShare:Number($("opportunityShare").value)||0,dvpMin:Number($("dvpMin").value)||0,dvpSample:Number($("dvpSample").value)||1,requireOpponentData:$("requireOpponentData").checked,position:$("positionFilter").value,market:$("marketFilter").value,side:$("sideFilter").value,team:$("teamFilter").value,opponent:$("opponentFilter").value,player:$("playerFilter").value.trim().toLowerCase(),legOddsMin:Number($("legOddsMin").value),legOddsMax:Number($("legOddsMax").value),parlayOddsMin:Number($("parlayOddsMin").value),parlayOddsMax:Number($("parlayOddsMax").value),legsMin:clamp(Number($("legsMin").value)||1,1,10),legsMax:clamp(Number($("legsMax").value)||1,1,10),uniquePlayers:$("uniquePlayers").checked,avoidSameGame:$("avoidSameGame").checked,weights:Object.assign({},state.weights)};
}
function analyze(row,cfg){
  const player=state.playerByName.get(norm(row.player));if(!player)return null;
  const odds=Number(row.odds);if(!Number.isFinite(odds)||odds<cfg.legOddsMin||odds>cfg.legOddsMax)return null;
  ...
  const usage=state.usage.get(String(player.id))||{target:0,carry:0,opportunity:0};
  if(usage.target<cfg.targetShare||usage.carry<cfg.carryShare||usage.opportunity<cfg.opportunityShare)return null;
  const spec=metricSpec(row),metric=dvpMetric(spec),dvpRow=state.dvp.get(opp+"|"+pos),dvpInfo=metric&&dvpRow&&dvpRow.metrics[metric],dvpPct=dvpInfo&&dvpInfo.percentile;
  if(cfg.requireOpponentData&&(!dvpRow||dvpRow.samples<cfg.dvpSample||!Number.isFinite(dvpPct)))return null;
  if(cfg.dvpMin>0&&(!Number.isFinite(dvpPct)||dvpPct<cfg.dvpMin))return null;
  if(cfg.dvpMin>0&&dvpRow&&dvpRow.samples<cfg.dvpSample)return null;
  ...
  const usageSignal=clamp(Math.max(usage.target,usage.carry,usage.opportunity)/55,0,1),matchup=Number.isFinite(dvpPct)?dvpPct/100:.5,modelProb=clamp(.38*recent+.32*season+.12*h2h+.10*usageSignal+.08*matchup,.03,.97),book=implied(odds)??.5,edge=modelProb-book;
  const w=cfg.weights,total=Object.values(w).reduce((a,b)=>a+b,0)||1,score=100*(w.recent*recent+w.season*season+w.h2h*h2h+w.usage*usageSignal+w.matchup*matchup+w.value*clamp(.5+edge*2,0,1))/total;
  return{row:row,player:player,pos:pos,team:team,opp:opp,market:market,rates:rates,usage:usage,dvpRow:dvpRow,dvpPct:dvpPct,dvpMetric:metric,modelProb:modelProb,impliedProb:book,edge:edge,score:clamp(score,0,100)};
}
...
function recalc(){
  ...
  const out=[];for(const row of state.odds){const x=analyze(row,cfg);if(x)out.push(x)}out.sort((a,b)=>b.score-a.score||b.edge-a.edge);state.eligible=out;state.slips=generateSlips(out,cfg);renderSummary();renderSlips();renderSignals();renderFormula();
}
...
function syncLabels(){for(const [k] of HIT_LABELS)$(k+"Value").textContent=$(k+"Min").value+"%";$("targetShareValue").textContent=$("targetShare").value+"%";$("carryShareValue").textContent=$("carryShare").value+"%";$("opportunityShareValue").textContent=$("opportunityShare").value+"%";$("dvpValue").textContent=$("dvpMin").value+"th+";$("dvpSampleValue").textContent=$("dvpSample").value+"+"}
...
function bind(){
  ...
  $("resetModel").addEventListener("click",()=>{for(const [k] of HIT_LABELS)$(k+"Min").value=DEFAULTS[k];for(const id of ["targetShare","carryShare","opportunityShare","dvpMin"])$(id).value=DEFAULTS[id];$("dvpSample").value=DEFAULTS.dvpSample;$("requireOpponentData").checked=false;for(const id of ["positionFilter","marketFilter","sideFilter","teamFilter","opponentFilter","playerFilter"])$(id).value="";for(const id of ["legOddsMin","legOddsMax","parlayOddsMin","parlayOddsMax","legsMin","legsMax"])$(id).value=DEFAULTS[id];$("uniquePlayers").checked=true;$("avoidSameGame").checked=false;state.weights=Object.assign({},DEFAULTS.weights);document.querySelectorAll("[data-weight]").forEach(b=>{const k=b.dataset.weight;b.querySelector("strong").textContent=state.weights[k];b.classList.add("active")});syncLabels();recalc()});
}
async function init(){
